...
(function() {
  window.getSimulateURL = function() {
    var scope;
    scope = $('html').scope();
  ...
  window.simStartLegacy = function() {
    return $.post(getSimulateURL(), JSON.stringify({
      has_refresh: true,
      version: "1"
    })).done(function(results) {
      return window.updateResultsLegacy(results);
    });
  };

  window.sendUpdateLegacy = function(postData) {
    postData['version'] = "1";
    return $.post(getSimulateURL(), JSON.stringify(postData)).done(function(results) {
      window.resetForm();
      return window.updateResultsLegacy(results);
    });
  };

  window.updateResultsLegacy = function(data) {
    var attachment, direction, ele, ele_quick_replies, i, j, len, level, media_type, media_url, media_viewer_elt, metadata, model, msg, parts, quick_replies, ref, reply, ussd;
    ...
    while (i < data.messages.length) {
      ...
      if (metadata && (metadata.quick_replies != null)) {
        quick_replies = "<div id='quick-reply-content'>";
        ref = metadata.quick_replies;
      ...
      if (media_type && media_viewer_elt) {
    ...
    $(".btn.quick-reply").on("click", function(event) {
      var payload;
      payload = event.target.innerText;
      return window.sendSimMessage(payload);
    });
    return window.updateActivity(data);
  };
