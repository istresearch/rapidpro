...
RUN mkdir -p /etc/nginx/env_vars.d \
 && mkdir -p /opt/code2use/nginx-in_events.d
...
#docker build -f docker/nginx/nginx.dockerfile . --build-arg STATICFILES_FROM_IMAGE="istresearch/p4-engage:ci-4.0.16.7-debian" -t "nginx-engage:v2"
#docker build -f docker/nginx/nginx.dockerfile . --build-arg STATICFILES_FROM_IMAGE="engage-local" -t "nginx-engage:v2"
